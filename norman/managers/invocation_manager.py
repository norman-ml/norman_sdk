...
class InvocationManager:
    def __init__(self) -> None:
        ...
        self.__retrieve_service = Retrieve()
    ...
    async def invoke(self, invocation_config: dict[str, Any]) -> dict[str, bytes]:
        ...
        async with self.__http_client:
            model, invocation = await self.__create_invocation_and_select_model(self.__authentication_manager.access_token, validated_invocation_config)
            await self.__upload_inputs(self.__authentication_manager.access_token, model, invocation, validated_invocation_config)
            await self.__wait_for_flags(self.__authentication_manager.access_token, invocation)
            output_handlers = await self.__get_response_handlers(self.__authentication_manager.access_token, invocation)
    ...
    async def __select_model_from_database(self, token: Sensitive[str], invocation_config: InvocationConfig) -> ModelProjection:
        ...
        models = await self.__persist_service.models.get_model_projections(token=token, constraint=model_select_constraint)
    ...
    async def __create_invocation_in_database(self, token: Sensitive[str], invocation_config: InvocationConfig) -> Invocation:
        ...
        invocations = await self.__persist_service.invocations.create_invocations_by_model_names(token=token, model_name_counter=model_name_counter)
        if invocations is None or len(invocations) == 0:
            raise RuntimeError("Invocation creation failed")

        return invocations[0]

    async def __upload_inputs(self, token: Sensitive[str], invocation: Invocation, invocation_config: InvocationConfig) -> None:
    ...
    async def __handle_input_upload(self, token: Sensitive[str], invocation_input: InvocationSignature, input_config: InvocationInputConfig) -> None:
        ...
        if input_source == InputSource.Primitive:
        ...
        elif input_source == InputSource.Stream:
            await self.__upload_stream_input(token, invocation_input, data)
        elif input_source == InputSource.Link:
    ...
    async def __upload_primitive_input(self, token: Sensitive[str], invocation_input: InvocationSignature, data: Any) -> None:
        byte_buffer = self.__file_transfer_service.normalize_primitive_data(data)
        buffer_size = self.__file_utils.get_buffer_size(byte_buffer)
        ...
        await self.__file_transfer_service.upload_from_buffer(token, pairing_request, byte_buffer)
    ...
    async def __submit_link_input(self, token: Sensitive[str], invocation_input: InvocationSignature, link: str) -> None:
        ...
        await self.__file_pull_service.submit_input_links(token, download_request)
    ...
    async def __get_response_handlers(self, token: Sensitive[str], invocation: Invocation) -> dict[str, ResponseHandler]:
        ...
        for output in invocation.outputs:
            invocation_output = self.__retrieve_service.get_invocation_output(token, invocation.account_id, invocation.version_id, invocation.id, output.id)
