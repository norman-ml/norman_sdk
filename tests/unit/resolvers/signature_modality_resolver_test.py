...
class TestSignatureModalityResolver:
    UNKNOWN_ENCODING_ERROR = "Unknown signature encoding"
    ...
    def test_resolve_audio_encoding_returns_audio(self, encoding: str) -> None:
        data_modality = SignatureModalityResolver.resolve(encoding)
    ...
    def test_resolve_handles_case_and_whitespace(self, encoding: str) -> None:
        ...
        assert data_modality in [DataModality.Audio, DataModality.Video]
    ...
    def test_resolve_none_raises_value_error(self) -> None:
        with pytest.raises(ValueError, match="encoding must be a non-empty string"):
            SignatureModalityResolver.resolve(None)  # type: ignore[arg-type]
    ...
    def test_resolve_non_string_raises_value_error(self, invalid_input: Any) -> None:
        with pytest.raises(ValueError, match="encoding must be a non-empty string"):
            SignatureModalityResolver.resolve(invalid_input)  # type: ignore[arg-type]
